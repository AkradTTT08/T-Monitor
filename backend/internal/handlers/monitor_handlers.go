...
func GetMonitorLogs(c *fiber.Ctx) error {
	...
	query := database.DB.Preload("API").Order("checked_at desc").Limit(200)

	// If admin, they see all logs.
	// If user, they only see logs for APIs matching their projects.
	if role != "admin" {
		query = query.Joins("JOIN apis ON apis.id = monitor_logs.api_id AND apis.deleted_at IS NULL").
			Joins("JOIN projects ON projects.id = apis.project_id").
			Where("projects.user_id = ?", userID)
	} else {
		// Admin: still exclude deleted APIs from the log list
		query = query.Joins("JOIN apis ON apis.id = monitor_logs.api_id AND apis.deleted_at IS NULL")
	}
