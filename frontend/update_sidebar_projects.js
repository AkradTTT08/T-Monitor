...
content = content.replace(
    `            {#if !isSidebarCollapsed}
              <div class="flex flex-col min-w-0 transition-opacity">
                ...
                >
                <span
                  class="text-[10px] uppercase font-bold tracking-wider text-slate-400 truncate mt-0.5"
                  >Enterprise</span
                >
              </div>
            {/if}
    ...
    switcherUI
).replace(/justify-between'} p-1.5/g, "flex-col items-start'} p-1.5").replace(/flex items-center gap-3 overflow-hidden/g, "flex items-center justify-between w-full gap-3 overflow-hidden"); // fix flex dir
...
content = content.replace(
    ...
    `$page.params.id && currentPath === \`/dashboard/projects/\${$page.params.id}\` && !$page.url.pathname.includes('/notifications')`
);

// 5. Add Projects Admin link
const projectsAdminLink = `
          <!-- Projects Admin Link -->
          <a href="/dashboard/projects" on:click={() => isMobileMenuOpen = false} title="Projects" class="w-full flex items-center {isSidebarCollapsed ? 'justify-center px-0' : 'justify-between px-3'} py-3 rounded-xl text-sm font-semibold transition-all {currentPath === '/dashboard/projects' && !$page.params.id ? 'bg-[#ecff82] text-slate-900 shadow-sm' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900'}">
            <div class="flex items-center gap-3">
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" class="{currentPath === '/dashboard/projects' && !$page.params.id ? 'text-slate-900' : 'text-slate-500'}" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>
