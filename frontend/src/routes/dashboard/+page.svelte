  ...
  async function handleCreateProject() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (coverFile) {
          await uploadCoverImage(project.id, coverFile);
        }
      ...
      const res = await fetch(
        `${API_BASE_URL}/api/v1/projects/${editingProjectId}`,
        {
          ...
          body: JSON.stringify({
            ...
            cover_position: editProjectCoverPos,
          }),
        },
      );
      if (res.ok) {
        if (editCoverFile) {
          await uploadCoverImage(editingProjectId, editCoverFile);
        }
        showEditModal = false;
        editCoverFile = null;
        window.location.reload();
...
<div class="fade-in">
  ...
  {:else if projects.length === 0}
    ...
    >
      ...
      >
        INITIALIZE A PROJECT WORKSPACE TO START GROUPING YOUR APIS FOR
  ...
  {:else}
    ...
    >
      {#each projects as project}
        <div
          class="bg-slate-800/40 backdrop-blur-xl rounded-3xl border border-slate-700/50 p-6 shadow-[0_8px_30px_rgb(0,0,0,0.5)] transition-all duration-500 hover:shadow-[0_0px_30px_rgba(6,182,212,0.15)] hover:border-cyan-500/40 hover:-translate-y-1 flex flex-col group relative overflow-hidden"
        >
          <div
          ...
          <!-- Project Cover Image -->
          <div
            class="h-32 -mx-6 -mt-6 mb-4 relative overflow-hidden bg-slate-900/50"
          >
            ...
            {/if}
            <div
              class="absolute inset-0 bg-gradient-to-t from-slate-800/80 to-transparent"
            ></div>
          </div>
          <div class="flex justify-between items-start mb-5 relative z-10">
            <div
              class="p-3 bg-slate-900 border border-slate-700/80 text-cyan-500/80 rounded-xl group-hover:border-cyan-500/50 group-hover:text-cyan-400 group-hover:shadow-[0_0_15px_rgba(6,182,212,0.3)] transition-all duration-300 relative overflow-hidden"
            >
              <div
                class="absolute inset-0 bg-cyan-400/20 mix-blend-screen filter blur-md opacity-0 group-hover:opacity-100 transition-opacity duration-300"
              ></div>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="24"
            ...
            <button
              ...
              class="text-slate-500 hover:text-cyan-400 p-2 rounded-lg hover:bg-slate-800 transition-colors"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="20"
                ...
                ><circle cx="12" cy="12" r="1.5"></circle><circle
                  ...
                  cy="12"
                  r="1.5"
                ></circle><circle cx="5" cy="12" r="1.5"></circle></svg
              >
            </button>
            {#if activeDropdownId === project.id}
              <div
                class="absolute right-0 top-10 mt-1 w-36 bg-slate-800 rounded-xl shadow-xl border border-slate-700 py-1 z-20 animate-in slide-in-from-top-2 duration-150"
                on:click|stopPropagation
              >
                <button
                  on:click={() => openEditModal(project)}
                  class="w-full text-left px-4 py-2 text-sm font-medium text-slate-300 hover:text-cyan-400 hover:bg-slate-700/50 transition-colors font-mono tracking-wide"
                  >EDIT</button
                >
                <div class="h-px w-full bg-slate-700/50 my-1"></div>
                <button
                  on:click={() => openDeleteModal(project)}
                  class="w-full text-left px-4 py-2 text-sm font-medium text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-colors font-mono tracking-wide"
                  >TERMINATE</button
                >
              </div>
            {/if}
          ...
          <h3
            class="text-xl font-bold text-cyan-50 mb-2 truncate font-mono tracking-wide relative z-10"
          >
            {project.name}
          </h3>
          <p
            class="text-slate-400/80 text-sm mb-6 flex-1 line-clamp-2 font-mono relative z-10"
          >
            {project.description || "NO_DESCRIPTION_PROVIDED"}
          </p>

          <div
            class="pt-5 border-t border-slate-700/50 flex justify-between items-center relative z-10"
          >
            <span
            ...
            <a
              ...
              class="text-sm font-bold text-cyan-500 hover:text-cyan-300 flex items-center gap-1.5 group-hover:underline font-mono tracking-wider transition-colors"
            >
              ACCESS
              <svg
                xmlns="http://www.w3.org/2000/svg"
...
<Modal bind:open={showCreateModal} title="Create Project">
  <form on:submit|preventDefault={handleCreateProject} class="space-y-5">
    <div>
      <label for="name" class="block text-sm font-medium text-cyan-50 mb-1"
        >Project Name</label
      >
      <input
        id="name"
        ...
        required
        class="w-full px-4 py-3 rounded-xl border border-slate-700/50 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all text-sm"
        placeholder="e.g. E-Commerce Core API"
      />
    </div>
    <div>
      <label for="desc" class="block text-sm font-medium text-cyan-50 mb-1"
        >Description</label
      >
      <textarea
        id="desc"
        ...
        class="w-full px-4 py-3 rounded-xl border border-slate-700/50 bg-slate-900/50 text-cyan-50 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all text-sm resize-none"
        placeholder="Brief description of this workspace..."
      ></textarea>
    </div>
    <div>
      <label for="cover" class="block text-sm font-medium text-cyan-50 mb-1"
        >Cover Image</label
      >
      <input
        id="cover"
        ...
        accept="image/*"
        on:change={(e) =>
          (coverFile = (e.target as HTMLInputElement).files?.[0] || null)}
    ...
    {#if coverFile}
      <div>
        <label for="pos" class="block text-sm font-medium text-cyan-50 mb-1"
          >Position Adjustment (Vertical: {newProjectCoverPos}%)</label
        >
        <input
          id="pos"
          type="range"
    ...
    <div class="pt-2 flex gap-3">
      <button
        ...
        class="flex-1 px-4 py-3 rounded-xl font-medium text-slate-400 bg-slate-800 hover:bg-slate-200 transition-colors"
        >Cancel</button
      >
      <button
        type="submit"
        class="flex-1 px-4 py-3 rounded-xl font-medium text-cyan-50 bg-cyan-600 hover:bg-blue-700 transition-colors"
        >Create</button
      >
    </div>
  </form>
...
<Modal bind:open={showEditModal} title="Edit Project">
  <form on:submit|preventDefault={handleEditProject} class="space-y-5">
    <div>
      <label for="edit_name" class="block text-sm font-medium text-cyan-50 mb-1"
        >Project Name</label
      >
      <input
        id="edit_name"
    ...
    <div>
      <label for="edit_desc" class="block text-sm font-medium text-cyan-50 mb-1"
        >Description</label
      >
      <textarea
        id="edit_desc"
    ...
    <div>
      <label
        for="edit_cover"
        class="block text-sm font-medium text-cyan-50 mb-1"
      ...
      <input
        ...
        accept="image/*"
        on:change={(e) =>
          (editCoverFile = (e.target as HTMLInputElement).files?.[0] || null)}
        class="w-full px-4 py-2 rounded-xl border border-slate-700/50 bg-slate-900/50 text-cyan-50/70 text-xs focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all"
      />
    </div>
    <div class="space-y-1">
      <label for="edit_pos" class="block text-sm font-medium text-cyan-50"
        >Position Adjustment (Vertical: {editProjectCoverPos}%)</label
      >
      <input
        id="edit_pos"
        type="range"
        min="0"
        max="100"
        bind:value={editProjectCoverPos}
        class="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-cyan-500"
      />
    </div>
    <div class="pt-2 flex gap-3">
...
<Modal bind:open={showDeleteModal} title="Delete Project">
  <div class="space-y-5">
    <p class="text-slate-400 text-sm">
      Are you sure you want to delete <span class="font-bold text-cyan-50"
    ...
    <div class="pt-2 flex gap-3">
      ...
      <button
        ...
        class="flex-1 px-4 py-3 rounded-xl font-medium text-cyan-50 bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-700 transition-colors"
