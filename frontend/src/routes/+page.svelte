<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (!res.ok) {
        error = data.error || 'Authenication failed';
      } else {
...
<div class="glass-panel w-full max-w-md p-8 rounded-2xl relative z-10 fade-in">
  <div class="text-center mb-8">
    <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-blue-100/50 mb-4 shadow-inner">
      <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 text-blue-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2v20"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
    </div>
    <h1 class="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-700 to-cyan-500">
  ...
  {#if error}
    <div class="bg-red-50 text-red-600 text-sm p-3 rounded-lg border border-red-100 mb-4 animate-pulse">
      {error}
    </div>
  {/if}
  ...
  <form on:submit|preventDefault={handleSubmit} class="space-y-5">
    ...
    <div>
      ...
      />
      {#if !isLogin}
        <p class="text-xs text-slate-400 mt-2 ml-1">Must be at least 8 chars with 1 uppercase, 1 lowercase, 1 number, and 1 special char.</p>
      {/if}
    </div>
  ...
  <div class="mt-6 text-center text-sm text-slate-500">
    <button type="button" on:click={() => { isLogin = !isLogin; error = ''; }} class="text-blue-600 hover:text-blue-800 font-medium hover:underline transition-colors">
