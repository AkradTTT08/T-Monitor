...
<div
  class="relative w-full h-full flex items-center bg-transparent group font-mono text-sm"
>
  <!-- Visual Overlay -->
  <div
    class="absolute inset-0 px-4 py-2 pointer-events-none whitespace-pre overflow-hidden text-transparent flex items-center"
    aria-hidden="true"
    style="z-index: 10;"
  >
    ...
    <div class="flex-1 w-full truncate">
      {#each segments as seg}
        {#if seg.type === "variable"}
          <span
            class="group/var relative inline-flex items-center transition-colors {!disabled
              ? seg.isValid
                ? 'text-emerald-500'
                : 'text-rose-500'
              : 'opacity-50'}"
          >
            ...
            {#if seg.isValid && !disabled}
              <div
                class="opacity-0 group-hover/var:opacity-100 transition-opacity absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-slate-900 border border-slate-700 text-cyan-50 shadow-[0_0_15px_rgba(6,182,212,0.3)] text-xs rounded break-words max-w-xs whitespace-normal z-50 pointer-events-none w-max font-sans tracking-normal font-normal"
              >
  ...
  <div
    class="absolute inset-0 px-4 py-2 pointer-events-none whitespace-pre overflow-hidden flex items-center z-10"
    aria-hidden="true"
  >
